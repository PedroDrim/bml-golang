...
func TestMeanAnalysis(t *testing.T) {

	t.Run("1. Instanciação - Deverá instanciar corretamente", func(t *testing.T) {
		instance := NewMeanAnalysis()
		if instance == nil {
			t.Error("1. Esperava instância não nula de NewMeanAnalysis")
		}
	})

	t.Run("2. Método Analysis() - Deverá retornar a media corretamente para uma lista válida", func(t *testing.T) {
		mock := NewMeanAnalysis()
